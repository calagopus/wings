...
pub struct ConfigureArgs {
    ...
    #[arg(
        long = "enroll",
        help = "a one-time enrollment code generated by the panel"
    )]
        ...
        Box::new(|env, arg_matches| {
            Box::pin(async move {
                ...
                if let Some(join_data) = args.join_data {
                    ...
                    Ok(0)
                } else if let Some(code) = args.enroll {
                    ...
                    let enrollment =
                        match crate::setup::redeem(&panel_url, &code, args.allow_insecure).await {
                            Ok(enrollment) => enrollment,
                            Err(err) => {
                                eprintln!(
                                    "{} {:#?}",
                                    "failed to enroll with the panel:".red(),
                                    err
                                );
                                return Ok(1);
                            }
                    ...
                    let allowed_origins = enrollment.merge_allowed_origins(
                        &env.as_ref()
                            .map(|config| config.load().allowed_origins.clone())
                            .unwrap_or_default(),
                    );
                    ...
                    let patch = serde_json::json!({
                        ...
                        "api": {
                            "port": enrollment.api_port,
                        },
                        "system": {
                            "sftp": {
                                "bind_port": enrollment.sftp_port,
                            },
                        },
                    });

                    let response = match apply(env.as_ref(), patch) {
                        Ok(response) => response,
                        Err(err) => {
                            eprintln!("{} {:#?}", "failed to apply enrollment:".red(), err);
                            return Ok(1);
                        }
                    };
